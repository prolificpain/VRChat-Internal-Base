...
internal class MakeMenu // Outside call, so we dont put everything on the main class (skids do this a lot, its messy ~w~)
{
    ...
    { // So, here we make the Make it internal, this means it can only be called withen the Example Namespace
        MainPage = new VRCPage("Example Menu"); // Making a new VRCPage, cause the we're assining a field, we can just use the new() keyword as the IDE already the new class we're making, basiclly the same as doing new VRCPage(..);
        ...
        new VRCToggle(MainGrp, "Test Tgl", (val /*here we have a var, this var will be the state of the toggle*/) => {
            if (val)
            { // you can write you own code here
